...
const ProductsTable = () => {
  ...
  return (
    <div>
      ...
      >
        {image ? <p>{image.name}</p> : <p>Drag and drop an image here</p>}
      </div>
      ...
      <table className="table">
        ...
        <tbody>
          {filteredProducts.map((product, index) => {
            const previousItem = index > 0 ? filteredProducts[index - 1].items : null;
            const shouldMerge = index === 0 || product.items !== previousItem;
            const rowSpan = shouldMerge ? filteredProducts.filter(p => p.items === product.items).length : 0;
